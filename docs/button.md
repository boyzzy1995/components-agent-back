...
:::demo 你可以设置 `dflay` 属性增加点击防抖功能。

```html
<fl-row>
  <fl-button :dflay="1000" type="primary" @click="handleClick1">1s 延迟 点击计数：{{ count1 }}</fl-button>
  <fl-button :dflay="3000" type="primary" @click="handleClick2">3s 延迟 点击计数：{{ count2 }}</fl-button>
</fl-row>
